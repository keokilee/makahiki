Namespace("org.wattdepot.makahiki");
  ...
  function makeHotSpotImageElement (data) {
    var view = transpose(data);
    var img = document.createElement('img');
    img.setAttribute('src', getChartUrl(view));
        ...
        function transpose(tempTable) {
          ...
          postTable.addColumn('number');
            ...
            for (k = 0; k < rows; k++) {
              ...
              addedrow.push( x );
              addedrow.push( y );
              addedrow.push( Number(temp.getValue(k,1).toFixed(0)) );
...
function getOptions(data) {
  ...
  options.chm = defaultMarker + getHighlightMarkers(data, maxSpotSize);
...
function getHighlightMarkers(data, maxSpotSize) {
  ...
  var testString = '|o,FF0000,1,13,20|o,FF0000,1,7,20';
  var xColumn = 2;
  var yColumn = 3
  var valueColumn = 2;
  var returnString = '';
...
function showTooltip(label, xCoord, yCoord) {
  ...
  tooltip.style.left = (120+ xCoord + 20) + 'px';
